...
contract("ChainPark", function (accounts) {
  ...
  const STAFF_ONLY_LOT_INDEXES = [2]

  before(async function () {
    this.cpInstance = await ChainPark.deployed();
    this.ubpcInstance = await UBPC.deployed();
    accounts.forEach(a => this.ubpcInstance.claim_airdrop({ from: a }));
  });
  ...
  it("should revert when parking at lot 0", async function () {
    ...
    it("should not let you park if already parked", async function () {
      const cpInstance = await ChainPark.deployed();
      try {
        await cpInstance.park(1, { from: accounts[0]});
        await cpInstance.park(3, { from: accounts[0]});
    ...
    it("should not let you park at a lot that is full", async function () {
      const cpInstance = await ChainPark.deployed();
      for (let i=0; i<8; i++) {
        await cpInstance.park(1, { from: accounts[i]});
      }
      try {
        await cpInstance.park(1, { from: accounts[5]});
    ...
    it("should update currentlyParked mapping when users park and leave", async function () {
      ...
      await cpInstance.park(3, { from: accounts[0]}); // park in lot 1
      assert.equal(await cpInstance.currentlyParked(accounts[0]), 3, "User should be parked in lot 3"); // check that user is parked in lot 1
      await cpInstance.leave({ from: accounts[0] }); // user leaves
    ...
    it("every lot should incriment on park and decrement on leave with one account", async function () {
      ...
      for (let i = 1; i < 27; i++) {
        ...
        await cpInstance.park(i, { from: accounts[0]});
    ...
    it("every lot should incriment on park and decrement on leave with many accounts", async function () {
      ...
      for (let lot = 1; lot < 27; lot++) {
        ...
        for (let account = 0; account < 5; account++) {
          await cpInstance.park(lot, { from: accounts[account]});
        }
